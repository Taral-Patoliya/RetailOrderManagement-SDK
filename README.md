...
$payloadFactory = new \Radial\RetailOrderManagement\Payload\PayloadFactory;
// Instantiate a payload object with the factory by passing
// the full class name of the payload to the factory.
$payload = $payloadFactory
    ->buildPayload('\Radial\RetailOrderManagement\Payload\Payment\StoredValueBalanceRequest');
...
try {
    $payload->validate();
} catch (\Radial\RetailOrderManagement\Payload\Exception\InvalidPayload $e) {
...
```php
/** @var \Radial\RetailOrderManagement\Api\HttpApi $api */
$api;
...
// \Radial\RetailOrderManagment\Api\HttpApi object.
...
```php
/** @var \Radial\RetailOrderManagment\Payload\OrderEvents\OrderShipped $payload */
$payload;
...
if ($destination instanceof \Radial\RetailOrderManagement\Payload\OrderEvents\IMailingAddress) {
    $destination->getFistName();
    $destination->getLastName();
} elseif ($destination instanceof \Radial\RetailOrderManagement\Payload\OrderEvents\IStoreFrontDetails) {
...
$apiConfig = new \Radial\RetailOrderManagement\Api\HttpConfig(
...
$httpApi = new \Radial\RetailOrderManagement\Api\HttpApi($apiConfig);
...
} catch (\Radial\RetailOrderManagement\Payload\Exception\UnsupportedOperation $e) {
...
} catch (\Radial\RetailOrderManagement\Api\Exception\UnsupportedHttpAction $e) {
    // Exception may be thrown from: { send }
    // handle the case where the http method is configured with an invalid value
    print $e->getMessage();

} catch (\Radial\RetailOrderManagement\Api\Exception\NetworkException $e) {
    // Exception may be thrown from: { send }
...
while ($payloads->valid()) {
    ...
    } catch (\Radial\RetailOrderManagement\Payload\Exception\Payload $e) {
