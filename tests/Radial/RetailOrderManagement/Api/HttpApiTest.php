...
class HttpApiTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->requestPayloadStub = $this->getMock('Radial\RetailOrderManagement\Payload\IPayload');
        $this->replyPayloadStub = $this->getMock('Radial\RetailOrderManagement\Payload\IPayload');

        // create mocks for the config and api objects
        $this->configStub = $this->getMock(
            'Radial\RetailOrderManagement\Api\HttpConfig',
            null,
            ['key', 'host', 'major', 'minor', 'store', 'service', 'operation']
        );
        $this->apiStub = $this->getMock(
            'Radial\RetailOrderManagement\Api\HttpApi',
            ['sendRequest'],
            [$this->configStub]
        );
    ...
    /**
     * @expectedException \Radial\RetailOrderManagement\Api\Exception\NetworkError
     */
    ...
    public function testSendRequestHandlesAction($supported, $action)
    {
        ...
        if (!$supported) {
            $this->setExpectedException('\Radial\RetailOrderManagement\Api\Exception\UnsupportedHttpAction');
        }
    ...
    public function testLogPayloadMessage()
    {
        ...
        /** @var HttpApi */
        $api = $this->getMockBuilder('Radial\RetailOrderManagement\Api\HttpApi')
    ...
    {
        $apiClass = 'Radial\RetailOrderManagement\Api\HttpApi';
        $key = 'rom_request_body';
