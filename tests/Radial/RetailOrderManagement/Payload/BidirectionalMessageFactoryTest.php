...
class BidirectionalMessageFactoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    /** @var \Radial\RetailOrderManagement\Api\IConfig */
    protected $config;
    ...
    public function setUp()
    {
        ...
        $this->payload = $this->getMock('\Radial\RetailOrderManagement\Payload\IPayload');
        // this will be stubbed to return expected message type config keys
        $this->config = $this->getMock('\Radial\RetailOrderManagement\Api\IConfig');
        // stub the payload factory to always just return a payload
        $this->payloadFactory = $this->getMock('\Radial\RetailOrderManagement\Payload\IPayloadFactory');
        $this->payloadFactory->expects($this->any())
            ->method('buildPayload')
            ->will($this->returnValue($this->payload));
        $this->messageTypes = [
            self::SUPPORTED_MESSAGE_TYPE => [
                'request' => '\Radial\RetailOrderManagement\Payload\IPayload',
                'reply' => '\Radial\RetailOrderManagement\Payload\IPayload',
            ],
        ];
        $this->messageFactory =
